...
def get_gradcam_image(img_names, attributions, image, class_name, layer_name):
    for i in range(len(image)):
        ...
        cam_image_final = Image.fromarray(cam_image)
        path = os.path.join(RESULTS_ROOT, class_name, f"{name}_grad_cam_{layer_name}.jpg")
        cam_image_final.save(path)

def get_attributions(model, class_category, class_mask_float, image, layer):
    target_layers = [layer]
...
def plot_all_irof_curves(histories, class_name, layer_name):
    for history in histories:
        history = np.array(history)
        plt.plot(range(len(history)), history, marker='o')
    plt.title('AOC Curve')
    ...
    plt.grid(True)
    path = os.path.join(RESULTS_ROOT, class_name, f"all_irof_{layer_name}.jpg")
    plt.savefig(path)
    plt.close()

def plot_avg_irof_curve(histories, class_name, layer_name):
    # Step 1: Find the length of the longest list
    max_length = max(len(lst) for lst in histories)
    ...
    plt.ylabel('Class ' + class_name + ' Score')
    plt.grid(True)
    path = os.path.join(RESULTS_ROOT, class_name, f"avg_irof_{layer_name}.jpg")
    plt.savefig(path)
    plt.close()
    ...
    for layer, layer_name in zip(layers_to_try, layer_names):
        ...
        for i, gt_batch in enumerate(test_loader):
            ...
            for class_category in range(TASKS_NUM_CLASS[0]):
                ...
                attributions = get_attributions(model, class_category, class_mask_float, image, layer)
                ...
                if valid_indices:
                    ...
                    attributions = get_attributions(model, class_category, class_mask_float, x_batch, layer)

                    get_gradcam_image(reduced_image_names, a_batch, x_batch, class_name, layer_name)
        ...
        all_scores[layer_name] = class_scores
        all_histories[layer_name] = class_histories

        for category in class_scores.keys():
            class_name = sem_idx_to_class[category]
            plot_all_irof_curves(class_histories[category], class_name, layer_name)
            plot_avg_irof_curve(class_histories[category], class_name, layer_name)

        with open(os.path.join(RESULTS_ROOT, layer_name, 'histories.pkl'), 'wb') as file:
            pickle.dump(all_histories[layer_name], file)
        
        with open(os.path.join(RESULTS_ROOT, layer_name, 'scores.pkl'), 'wb') as file:
            pickle.dump(all_scores[layer_name], file)
