...
tar xzf /scratch/s4716671/pruned_models/pt/70/baseline1/results.tar.gz -C $TMPDIR/pt
...
# Run training
python3 launch_training.py --dataset nyuv2 --method disparse_pt --ratio 70 --dest $TMPDIR/results --source $TMPDIR/pt

############ SAVING:

# Save models by compressing and copying from TMPDIR
tar czvf /scratch/$USER/pruned_models/pt/70/baseline1/trained_results.tar.gz $TMPDIR/results