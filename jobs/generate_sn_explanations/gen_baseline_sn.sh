#!/bin/bash
#SBATCH --job-name=explanations_sn_baselines
...
# Run training
python3 generate_all_explanations_hbrk.py --method baseline --task sn

########### GET RESULTS

# Save models by compressing and copying from TMPDIR
mkdir -p /scratch/$USER/explanations/all/sn/baseline/job_${SLURM_JOBID}
tar czvf /scratch/$USER/explanations/all/sn/baseline/job_${SLURM_JOBID}/results.tar.gz $TMPDIR/results