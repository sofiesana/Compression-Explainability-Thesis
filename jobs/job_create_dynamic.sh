...
# Run training
python3 create_pruned_net.py --dataset nyuv2 --method disparse_dynamic --ratio 50 --dest $TMPDIR/results

# Save models by compressing and copying from TMPDIR
mkdir -p /scratch/$USER/pruned_models/dynamic/50/job_${SLURM_JOBID}
tar czvf /scratch/$USER/pruned_models/dynamic/50/job_${SLURM_JOBID}/results.tar.gz $TMPDIR/results