...
# Run training
python3 irof_seg_all.py --head all --method static --task seg --irof mean --model_num 1

########### GET RESULTS

# Save models by compressing and copying from TMPDIR
mkdir -p /scratch/$USER/irof/all/seg/static/static1_${SLURM_JOBID}
tar czvf /scratch/$USER/irof/all/seg/static/static1_${SLURM_JOBID}/results.tar.gz $TMPDIR/results