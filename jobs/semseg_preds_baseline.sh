...
# Run training
python3 semseg_preds.py --head all --task seg --irof mean

########### GET RESULTS

# Save models by compressing and copying from TMPDIR
mkdir -p /scratch/$USER/seg_preds/baseline1/job_${SLURM_JOBID}
tar czvf /scratch/$USER/seg_preds/baseline1/job_${SLURM_JOBID}/results.tar.gz $TMPDIR/results