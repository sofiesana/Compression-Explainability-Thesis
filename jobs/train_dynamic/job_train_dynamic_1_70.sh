...
# Run training
python3 launch_training.py --dataset nyuv2 --method disparse_dynamic --ratio 70 --dest $TMPDIR/results --source $TMPDIR/pt

############ SAVING:

# Save models by compressing and copying from TMPDIR
mkdir -p /scratch/$USER/pruned_models/dynamic/70/dynamic1
tar czvf /scratch/$USER/pruned_models/dynamic/70/dynamic1/trained_results.tar.gz $TMPDIR/results