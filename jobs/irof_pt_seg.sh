...
# Run training
python3 irof_seg_all.py --head all --method pt --task seg --irof mean

########### GET RESULTS

# Save models by compressing and copying from TMPDIR
mkdir -p /scratch/$USER/irof/all/seg/pt/job_${SLURM_JOBID}
tar czvf /scratch/$USER/irof/all/seg/pt/job_${SLURM_JOBID}/results.tar.gz $TMPDIR/results