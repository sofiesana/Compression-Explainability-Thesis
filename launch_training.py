...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--method', type=str, help='method name: baseline, disparse_static, disparse_pt, disparse_dynamic', default="disparse_static")
    ...
    if dataset == "nyuv2":
        ...
        train_dataset = NYU_v2(
            DATA_ROOT, 'train', crop_h=CROP_H, crop_w=CROP_W)
        train_loader = DataLoader(
            train_dataset, batch_size=BATCH_SIZE, num_workers=8, shuffle=True, pin_memory=True)
        val_dataset = NYU_v2(DATA_ROOT, 'val')
        val_loader = DataLoader(
            val_dataset, batch_size=1, num_workers=8, shuffle=True, pin_memory=True)
        ...
        train_loader = DataLoader(
            train_dataset, batch_size=BATCH_SIZE, num_workers=8, shuffle=True, pin_memory=True)
        val_dataset = CityScapes(DATA_ROOT, 'val')
        val_loader = DataLoader(
            val_dataset, batch_size=1, num_workers=8, shuffle=True, pin_memory=True)
    elif dataset == "taskonomy":
        from config_taskonomy import *
        train_dataset = Taskonomy(
            DATA_ROOT, 'train', crop_h=CROP_H, crop_w=CROP_W)
        train_loader = DataLoader(
            train_dataset, batch_size=BATCH_SIZE, num_workers=8, shuffle=True, pin_memory=True)
        val_dataset = Taskonomy(DATA_ROOT, 'val')
        val_loader = DataLoader(
            val_dataset, batch_size=BATCH_SIZE, num_workers=8, shuffle=False, pin_memory=True)
    else:
    ...
    if pruned:
        print(source)
        save_path = source + '/tmp/results/best_nyuv2_baseline.pth')
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer, step_size=DECAY_LR_FREQ, gamma=DECAY_LR_RATE)

    batch_update = 16
    if method == "disparse_dynamic":
        D = (100 - ratio) / 100
        # prune_rate, end, interval, init_lr, weight_decay, tasks_num_class, tasks
        config_dict = {"prune_rate": PRUNE_RATE, "end": END, "interval": INT, "init_lr": INIT_LR, "weight_decay": WEIGHT_DECAY,
                       "tasks_num_class": TASKS_NUM_CLASS, "tasks": TASKS, "decay_freq": DECAY_LR_FREQ, "decay_rate": DECAY_LR_RATE}
        ...
        net = disparse_dynamic_train(net, dataset, criterion, amp_criterion, optimizer, scheduler, train_loader, val_loader,
                                     network_name, batch_update, D, config_dict, max_iters=MAX_ITERS, save_model=True, log_file=log_file, method=method)
    elif not pretrained:
        ...
        optimizer = torch.optim.Adam(
            net.parameters(), lr=RETRAIN_LR, weight_decay=WEIGHT_DECAY)
        net = train(net, dataset, criterion, optimizer, scheduler, train_loader, val_loader, network_name,
                    batch_update, max_iters=RETRAIN_EPOCH, save_model=True, log_file=log_file, method=method, dest=dest)
    ...
    net.eval()
    res = evaluator.get_final_metrics(net, val_loader)
