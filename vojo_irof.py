...
# Add the path to your forked library to the Python path
sys.path.insert(0, os.path.abspath('D:/Documents/University/XAI/Quantus-Thesis-Version'))

from quantus import IROF
from quantus import functions

def plot_all_irof_curves(histories, task, class_name = None):
    path = None
    ...
    if class_name is not None:
        ...
        path = os.path.join("Irof_results/" + task + "/", str(class_name) + ' all_irof.png')
    else:
        plt.ylabel('Class Score')
        path = os.path.join("Irof_results/" + task + "/", 'all_irof.png')
    os.makedirs(path, exist_ok=True)
    plt.savefig(path)
    print("Saved at: ", path)
    plt.close()

def plot_avg_irof_curve(histories, task, class_name = None):
    path = None
    ...
    if class_name is not None:
        if task == "sn":
            plt.ylabel('Surface Normal Octant \'' + str(class_name) + '\' Score')
        path = os.path.join("Irof_results/" + task + "/", str(class_name) + ' avg_irof.png')
    else:
        plt.ylabel('Score')
        path = os.path.join("Irof_results/" + task + "/", 'avg_irof.png')
    os.makedirs(path, exist_ok=True)
    plt.savefig(path)
    print("Saved at: ", path)
    plt.close()
...
def run_irof_surf_norm(model, model_name, test_loader, location, sem_idx_to_class, device, num_images_to_gen_irof = 200):
    ...
    for i, gt_batch in enumerate(test_loader):
        ...
        for norm_num in range(9):
            # not working because the test loader is loading different files than the ones used to generate the two examples. need to retrieve them from harbok
    ...
    for category in class_scores.keys():
        ...
        plot_all_irof_curves(class_histories[category], "sn", class_name)
        plot_avg_irof_curve(class_histories[category], "sn", class_name)
...
def irof_caller(model, model_name, test_loader, location, device):
    ...
    with torch.no_grad():
        for task in tasks:
            ...
            os.makedirs(path, exist_ok=True)
            with open(os.path.join("Irof_results/" + task + "/", model_name + 'histories.pkl'), 'wb') as file:
                pickle.dump(histories, file)
            
            with open(os.path.join("Irof_results/" + task + "/", model_name + 'scores.pkl'), 'wb') as file:
